...
本拡張は `document_start` + `MAIN` ワールドでプレイヤーの音量変化を監視します。
YouTubeの音量UIや `video.volume` の標準 getter / setter は上書きせず、正規化で実音量が下がった後に UI スライダーの `aria-valuenow` へ再同期します。これにより、ユーザーの音量スライダー操作を壊さずにラウドネス正規化だけを打ち消します。
